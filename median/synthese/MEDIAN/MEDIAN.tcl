if { ![project_exists {MEDIAN}] } {
     project_new {MEDIAN}
} else {
     project_open {MEDIAN}
}
set_project_settings -cmp {MEDIAN}
if { ![project_settings_exist -sim MEDIAN] } {
        set_project_settings -sim MEDIAN
}
set_global_assignment -section_id {MEDIAN} -name {EDA_DESIGN_ENTRY_SYNTHESIS_TOOL} {PRECISION SYNTHESIS}
set_global_assignment -section_id {MEDIAN} -name {EDA_SIMULATION_TOOL} {MODELSIM (VERILOG HDL OUTPUT FROM QUARTUS II)}
set_global_assignment -name {VQM_FILE} {MEDIAN.vqm}
...
if { [file exists {MEDIAN_altera.prop}] } {
  source MEDIAN_altera.prop
}
set_global_assignment -name {FAMILY} {CYCLONE II}
set_global_assignment -section_id {MEDIAN} -name {DEVICE} {EP2C35F672C8}
set_global_assignment -name {PROJECT_SHOW_ENTITY_NAME} {OFF}
set_global_assignment -name {SDC_FILE} {MEDIAN_pnr_constraints.sdc}
